...
using NitroInterceptor.Message.Numerics;

namespace NitroInterceptor.Message
{
    public class NMessage
    {
        ...
        public NMessage(short header, params object[] values)
            : this(Construct(header, values))
        {
            _written.AddRange(values);
        }

        public NMessage(string data)
            : this(ToBytes(data))
        { }

        public NMessage(byte[] data, bool isOutgoing = false)
            : this()
        {
